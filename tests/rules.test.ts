...
describe('getLuminance', () => {
  it('should calculate luminance for black (0, 0, 0)', () => {
    expect(getLuminance([0, 0, 0])).toBeCloseTo(0);
  });

  it('should calculate luminance for white (255, 255, 255)', () => {
...
describe('convertToRgb', () => {
  it('should convert hex color #000000 to RGB [0, 0, 0]', () => {
    expect(convertToRgb('#000000')).toEqual([0, 0, 0]);
  });

  it('should convert RGB color string "rgb(255, 0, 0)" to RGB array [255, 0, 0]', () => {
    expect(convertToRgb('rgb(255, 0, 0)')).toEqual([255, 0, 0]);
  });

  it('should convert HSL color string "hsl(0, 100%, 50%)" to RGB array [255, 0, 0]', () => {
...
describe('detectColorMode', () => {
  it('should detect hex color mode for #000000', () => {
    expect(detectColorMode('#000000')).toBe('hex');
  });

  it('should detect RGB color mode for rgb(255, 0, 0)', () => {
    expect(detectColorMode('rgb(255, 0, 0)')).toBe('rgb');
  });

  it('should detect HSL color mode for hsl(0, 100%, 50%)', () => {
    expect(detectColorMode('hsl(0, 100%, 50%)')).toBe('hsl');
  });

  it('should return "unknown" for an invalid format', () => {
